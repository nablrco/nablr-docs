...
## <Icon icon="shield-check" /> Network boundary
...
|---------|------|------------|
| `POST /validate` | First prompt after cache expiry | `device_id` (UUID), `Authorization: Bearer <key>` |
...
## <Icon icon="flame" /> Firewall allowlist
...
## <Icon icon="wifi-off" /> Airgap proof
...
## <Icon icon="hard-drive" /> Offline / airgap operation

After activating a key, paid assets are cached locally in a restricted-permission directory. Subsequent prompt generation requires zero network calls.

All cache files are `chmod 600` - readable only by the current user.

## <Icon icon="key" /> Key security
...
## <Icon icon="file-text" /> Audit logging
...
## <Icon icon="package-search" /> Dependency audit
