---
title: Agents
description: 18 expert personas covering every SDLC phase
icon: users
---

{/* STORY-DOCS-CLAIMS-001 FR-3: frontmatter persona count corrected to match
AVAILABLE_AGENTS (the two tables below already matched at 18 rows). */}

All persona prompts require a Pro plan or active trial. Free tier returns an upgrade prompt.

## <Icon icon="workflow" /> SDLC workflow agents
...
## <Icon icon="sparkles" /> Specialist agents
...
## <Icon icon="terminal" /> Invoking an agent
...
## <Icon icon="cog" /> How rules are loaded

When you invoke a paid agent, nablr fetches the relevant rule files from the license server and injects them into the prompt. Rules are cached locally after the first activation - subsequent calls are instant with zero network dependency.
